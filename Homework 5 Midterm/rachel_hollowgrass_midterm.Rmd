...
# Try anti-dodging and widening the bars.
...
```{r}
diamonds_expensive <- filter(diamonds, price > 10000, carat < 1.5) %>% select(price, carat) %>% arrange(order(price))
proportion <- count(diamonds_expensive) / count(diamonds)
count(diamonds_expensive)  
count(diamonds)  
proportion
```
